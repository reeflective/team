...
func NewManager(db *gorm.DB, log *logrus.Entry, appDir string) *Manager {
	...
	certs.GenerateCertificateAuthority(mtlsCA, "")
	certs.GenerateCertificateAuthority(userCA, "users")
...
func (c *Manager) MtlsC2ServerGenerateECCCertificate(host string) ([]byte, []byte, error) {
...
func (c *Manager) generateCertificate(caType string, subject pkix.Name, isCA bool, isClient bool, privateKey interface{}) ([]byte, []byte) {
	...
	if isCA {
		...
		caCert, caKey, err := c.GetCertificateAuthority(caType) // Sign the new certificate with our CA
		if err != nil {
